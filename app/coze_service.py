...
# ==========================================
# ЖЕСТКАЯ НАСТРОЙКА ПУТИ
# ==========================================
DB_DIR = "/data"
DB_PATH = os.path.join(DB_DIR, "conversations.db")

# Проверка: видим ли мы диск?
if not os.path.exists(DB_DIR):
    # Если мы здесь, значит Railway НЕ примонтировал диск по пути /data
    logger.error(f"КРИТИЧЕСКАЯ ОШИБКА: Папка {DB_DIR} не найдена! Проверь настройки Volumes в Railway.")
    # Принудительно создаем базу в текущей папке, чтобы бот хотя бы запустился для тестов, 
    # но в логах будет видна ошибка выше.
    DB_PATH = "conversations_temporary.db"
else:
    logger.info(f"Диск найден, база данных будет по пути: {DB_PATH}")
...
def ask_coze(user_id: str | int, message: str) -> str:
    ...
    if row:
        logger.info(f"ID найден в базе: {row[0]}")
        payload["conversation_id"] = row[0]
    else:
        logger.info(f"Для пользователя {user_id_str} в базе ничего нет.")
    ...
    try:
        ...
        return "".join(answer_parts) if answer_parts else "Ошибка получения ответа."
